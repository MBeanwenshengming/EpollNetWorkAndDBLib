...
void ConnectorThread::CheckIsLittleEndian()
{
	union
	{
		unsigned int  a;
...
int ConnectorThread::ThreadDoing(void* pParam)
{
	...
	while(1)
	{
		if (bSendable)
		{
			...
			unsigned short usSendDataAllToSend = usSendDataAll;

			memmove(szTempSendBuf, &usSendDataAllToSend, sizeof(unsigned short));
			memmove(szTempSendBuf + sizeof(unsigned short), packetToSend.GetDataBuf(), packetToSend.GetDataLen());
		...
		while (unRemainedDataLen > sizeof(unsigned short))
		{
			...
			unsigned short usDataLen = usPacketLen;
			...
			//printf("RecvPacketLen:%u, netorder:%u\n", usPacketLen, usDataLen);
			...
			if (unRemainedDataLen >= usPacketLen)
			{
				...
				memmove(szRecvedBuf, szRecvedBuf + usPacketLen, unRemainedDataLen);
